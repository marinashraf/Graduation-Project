...
export default function SystemHealthDashboard() {
  ...
  function updateMetrics() {
    const nextCpu = clamp(cpu + rand(-6, 8), 2, 98);
    const nextRam = clamp(ram + rand(-4, 6), 6, 98);
    const nextTemp = clamp(temperature + rand(-2, 3), 22, 95);
    const nextStorage = clamp(storageUsedPct + rand(-1, 1), 10, 98);
    const nextUpload = clamp(upload + rand(-3, 6), 0, 1000);
    const nextDownload = clamp(download + rand(-6, 10), 0, 1000);

    setCpu(Math.round(nextCpu));
    setRam(Math.round(nextRam));
    setTemperature(Math.round(nextTemp));
    setStorageUsedPct(Math.round(nextStorage));
    setUpload(Math.round(nextUpload));
    setDownload(Math.round(nextDownload));

    pushTrend(setCpuTrend, nextCpu);
    pushTrend(setRamTrend, nextRam);
    pushTrend(setNetTrend, (nextUpload + nextDownload) / 2);
  }

 function pushTrend(setter, value) {
  setter(prev => {
    const newData = [...prev, { value: Math.round(value) }];
  ...
  <div className="chart-container">
    <ResponsiveContainer width="100%" height={120}>
      <PieChart>
        <Pie
          data={[
            { name: "Used", value: temperature - 20 }, // افترضنا min 20°C
          ...
          dataKey="value"
        >
          <Cell key="used" fill="#ff4c4c" />
          <Cell key="remaining" fill="#444" />
        </Pie>
      </PieChart>
    </ResponsiveContainer>
  </div>
